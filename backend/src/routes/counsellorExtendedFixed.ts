...
// Helper function to get counsellor profile ID from user ID
const getCounsellorProfileId = async (userId: string) => {
  const user = await User.findById(userId);
  if (!user) return null;
  
  const counsellorProfile = await Counsellor.findOne({ email: user.email });
...
router.get('/all-bookings', authenticateToken, requireCounsellor, async (req: AuthRequest, res) => {
  try {
    ...
    const filter: any = { counsellorId: counsellorProfileId };
