...
const addYouTubeAudiosAuto = async () => {
  try {
    ...
    if (!YOUTUBE_API_KEY || YOUTUBE_API_KEY === 'YOUR_YOUTUBE_API_KEY') {
      ...
      // Use manual data instead
      await addManualAudioData(adminUser._id as mongoose.Types.ObjectId);
      return;
    }
    ...
    // Show summary
    const totalAudios = await Resource.countDocuments({ category: 'audio' });
...
// Fallback function for manual data when API key is not available
async function addManualAudioData(adminUserId: mongoose.Types.ObjectId) {
